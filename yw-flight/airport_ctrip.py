...
class CtripInfo(object):
  """docstring for ClassName"""
  def __init__(self):
    # super(ClassName, self).__init__()
    # self.arg = arg
    # self.proxy_ips = self.opentxt('./proxy.txt')
    self.proxy_ips = self.proxies()
    self.names = self.opentxt('./airport.txt')
    self.save_path = './update_data/'
    if not os.path.exists(self.save_path): os.mkdir(self.save_path)
    self.info_lst = []

  def opentxt(self,path):
    # 为了方便直接调用本地文件输入 nickname、ip_ports
    lst = []
  ...
  def soup2lst(self,html):
    ...
    for tr in trs:
      ...
      if isinstance(tr, bs4.element.Tag):
        ...
        self.info_lst.append(row)

  def saveData(self):
    col_name = ['航班号', '出发机场', '到达机场', '计划起降时间', '预估起降时间', '实际起降时间', '状态', '操作']
    pd.DataFrame(self.info_lst, columns=col_name).to_excel(self.xlsx_path, index=False, encoding='utf-8')

  def randomProxy(self):
    proxy_ip = np.random.choice(self.proxy_ips)
    proxies = {'http': "http://" + proxy_ip}
    # print(proxies)
    return proxies

  def spiderHtml(self,name):
    self.xlsx_path = self.save_path + name + '.xlsx'
    ...
    while Flag:
      try:
        ...
        print('cur city name:',name, chi_name,response.url)
